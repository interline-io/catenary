    ...
    <section class="section">
      ...
      >
        <template #notes>
          ...
          <p class="mt-2">
            A visually hidden status region announces each change together with the resulting selection (<em>Added Apple. Selected: Apple, Banana.</em>), since the input clears after selecting and the delta alone leaves users unsure what is held; empty filter results announce <em>No results</em>. The input is also described by a visually hidden usage hint (customizable via <code>aria-usage-hint</code>) explaining the type-to-search, arrow, and Enter/Tab interaction. At the <code>max-tags</code> limit the input stays enabled (disabling the focused element would drop keyboard focus); the option list empties, the placeholder changes, and the live counter reports the state. Removing a tag with its remove button moves focus to the next tag's remove button rather than dropping it.
          </p>
          <p class="mt-2">
