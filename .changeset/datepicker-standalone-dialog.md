...
- The weekday header row (`.cat-datepicker-weekdays`) moved inside `.cat-datepicker-days`, and week rows (`.cat-datepicker-row`) are now `display: grid` instead of `display: contents`.
