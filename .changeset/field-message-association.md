...
- `cat-input`, `cat-textarea`, `cat-select`, and `cat-slider` merge the field message id into their `aria-describedby` (alongside their own `ariaDescribedby` where supported) and render `aria-invalid="true"` when the field's variant, or their own `variant` prop, is `danger`.
