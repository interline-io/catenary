...
- **`cat-button`** — new `icon` prop renders an icon-only button (e.g. an addon button in a `has-addons` field) at the correct size, instead of slotting a bare `<cat-icon>` which rendered oversized and made the button taller than adjacent inputs/selects. The rendered glyph is `aria-hidden`; supply an accessible name via `aria-label` (a dev-mode warning fires if it is missing). `icon` is ignored when a default slot, `label`, `iconLeft`, or `iconRight` is present.
