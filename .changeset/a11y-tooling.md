...
- Adds a small in-house axe-core wrapper at `src/testutil/axe.ts` and an `expectNoAxeViolations()` helper in `testutil/component-helpers.ts` for asserting zero axe violations in component tests. The wrapper disables `cat.color` rules globally (jsdom can't compute styles) and stubs `HTMLCanvasElement.prototype.getContext` to silence jsdom's "Not implemented" warning on axe-core's icon-ligature precheck.
