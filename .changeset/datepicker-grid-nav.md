...
When focus crosses a month/year boundary the visible calendar follows along. The roving tab stop is clamped onto a selectable day, so calendars with `minDate` / `maxDate` / `unselectableDates` always have a focusable entry point.

The grid container gains `role="grid"` and an `aria-label` naming the visible month/year so screen readers announce the cursor's context. Day buttons gain `role="gridcell"` and `aria-selected`.
